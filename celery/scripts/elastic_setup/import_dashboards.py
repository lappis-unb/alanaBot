...
def importDashboards(pathToFile):
    ...
    if es_user is not None:
        ...
        requests.post(
            url=requestData[0], headers=requestData[1],
            auth=(es_user, es_pass), data=json.dumps(datas)
        )
    else:
        requests.post(
...
def createIndexPattern():
    ...
    if es_user is not None:
        es_pass = os.getenv("ELASTIC_PASS")
        requests.post(
            url=requestData[0],  auth=(es_user, es_pass),
            headers=requestData[1], data=json.dumps(datas)
        )
    else:
        requests.post(
            url=requestData[0], headers=requestData[1], data=json.dumps(datas)
