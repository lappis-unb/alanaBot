...
O usuário interage com o bot via Telegram, que manda as mensagens para o Rasa NLU através de
...
avaliação das conversas dos usuários e do bot.
...
sudo docker-compose up -d cronjob
```

# Cronjob

Esse serviço realiza a atualização da base de dados com os projetos de lei dos últimos dias e para o envio de notificações para usuários que foram cadastrados através da conversa com o bot. Para o seu funcionamento deve-se executar o seguinte comando:
```sh
sudo docker-compose up -d cronjob
```
...
Caso a documentação não esteja clara o suficiente, abra uma issue
