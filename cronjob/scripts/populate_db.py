...
class UpdateProjetos():
    ...
    def build_request_url(self, dict_palavras_chaves):
        ...
        proposicoes_url += "&dataInicio=" + request_dates["final_date"] +\
                           "&dataFim=" + request_dates["start_date"] +\
                           "&itens=100"
    ...
    def request_projeto(self, ids_projeto):
        headers = {"Content-Type": "application/json"}
    ...
    def get_page_number(self, page, splitted_url, pages):
        if page == "first":
            ...
            page_number = ''.join(char for char in raw_page_number if char.isdigit())
...
if __name__ == "__main__":
    TELEGRAM_DB_URI = os.getenv("TELEGRAM_DB_URI","")
    p = UpdateProjetos()
    json_data = p.fetch_palavras_chaves()
    lista = p.fetch_projetos(json_data)
    p.request_projeto(lista)